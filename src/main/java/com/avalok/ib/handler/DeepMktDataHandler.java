...
public class DeepMktDataHandler implements IDeepMktDataHandler {
	...
	protected final JSONArray publishOdbk = new JSONArray();
	protected final String publishChannel;

	private Consumer<Jedis> broadcastLambda;
	public DeepMktDataHandler(IBContract contract, boolean broadcast) {
		_contract = contract;
		publishChannel = "URANUS:"+contract.exchange()+":"+contract.currency()+"-"+contract.symbol()+":full_odbk_channel";
		// Pre-build snapshot
		publishOdbk.add(bids);
		publishOdbk.add(asks);
		publishOdbk.add(0); // Timestamp
		// Pre-build broadcast lambda.
		if (broadcast) {
			broadcastLambda = new Consumer<Jedis> () {
				@Override
				public void accept(Jedis t) {
					t.publish(publishChannel, JSON.toJSONString(publishOdbk));
				}
			};
		}
	}
...
//		}
		if (dataInited && broadcastLambda != null) {
			publishOdbk.set(0, bids);
			publishOdbk.set(1, asks);
			publishOdbk.set(2, System.currentTimeMillis());
